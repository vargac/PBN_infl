...
fn ibmfa_step(update_functions: &[Bdd], probs: &[f32], init_probs: &[f32],
              var_index: &HashMap<BddVariable, usize>) -> Vec<f32> {
    update_functions
        .iter()
        .enumerate()
        .map(|(i, update_function)|
...
fn ibmfa(update_functions: &[Bdd], probs: &mut Vec<Vec<f32>>,
         var_index: &HashMap<BddVariable, usize>, verbose: bool) {
    for i in 1..probs.len() {
        probs[i] = ibmfa_step(update_functions, &probs[i - 1], &probs[0],
                              var_index);
        if verbose {
            println!("{:?}", probs[i]);
        }
...
-> f32 {
    ...
    for _ in 0..iterations {
        probs = ibmfa_step(update_functions, &probs, init_probs, var_index);
        if verbose {
            println!("{:?}", probs);
        }
        ent = entropy(&probs);
...
fn main() {
    ...
    */
    ...
    let mut bdd_var_index = HashMap::new();
    ...
    for var_id in model.variables() {
        let update_function = &update_functions[var_id.to_index()];
        /*
        println!("{}: {}",
            model.get_variable_name(var_id),
            update_function.to_boolean_expression(context.bdd_variable_set()));
        for valuation in update_function.sat_clauses() {
            println!("{}", valuation_to_str(&valuation, &context));
        }
        println!("AFTER UNIT_BDD");
        */
        let support_set = update_function.support_set();
        let constrained_unit_bdd = unit_bdd
            .project(context
                .parameter_variables()
                .iter()
//                .chain(context.state_variables().iter())
                .filter(|&bdd_variable| !support_set.contains(bdd_variable))
                .copied()
                .collect::<Vec<_>>()
                .as_slice());
        for parametrization in constrained_unit_bdd.sat_clauses() {
            println!("{}", valuation_to_str(&parametrization, &context));
            let f = update_function.restrict(&parametrization.to_values());
            println!("\t{}",
        ...
        /*
        let constrained_update_function =
            constrained_unit_bdd.and(update_function);
        println!("{}", constrained_unit_bdd
            .to_boolean_expression(context.bdd_variable_set()));
        println!("{}", constrained_update_function
            .to_boolean_expression(context.bdd_variable_set()));
        for valuation in constrained_update_function.sat_clauses() {
            println!("{}", valuation_to_str(&valuation, &context));
        }
        */
        println!();
    ...
    println!("Entropy: {}",
        ibmfa_entropy(&update_functions, &probs[0], &bdd_var_index,
    ...
    while available_fixings.len() > 0 {
        ...
        let (min_entropy_index, min_entropy) = available_fixings
            ...
            .map(|&fixing| {
                ...
                let e = ibmfa_entropy(&update_functions, &init_probs,
